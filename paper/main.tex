...
\begin{enumerate}
\item \textbf{Geometry-Guided Candidate Generation:} We develop a Ghost Ball method that generates high-quality shot candidates by exploiting billiards geometry, reducing the candidate pool from 450 to 15 while preserving solution quality—a 30$\times$ reduction in candidate enumeration.
...
\caption{Overall decision pipeline: (1) heuristic/geometric Ghost Ball candidate generation, (2) fast filtering (1--2 rollouts), (3) refined evaluation (2--4 rollouts) with catastrophic foul filtering, (4) selective CMA-ES refinement (triggered only when score $< 60$; otherwise bypassed), (5) positional/safety shot planning, and (6) final verification (4--6 rollouts) with increased sampling in high-risk states.}
\label{fig:pipeline}
\end{figure}
...
\textbf{Heuristic-Guided Pruning:} Naive enumeration over all target balls $\times$ all pockets $\times$ velocity/spin variants yields $\sim$450 candidates per decision. We formalize pruning as a filtering pipeline to construct a reduced candidate set $C_{\text{pruned}} \subset C_{\text{initial}}$:
...
\begin{enumerate}
    \item Heuristic/Geometric Ghost Ball generation (15 candidates)
...
\textbf{Analysis:}
\begin{itemize}
    \item Geometric pruning is essential for wall-clock feasibility: as shown in the \textit{Over-180s Rate} column of Table~\ref{table_ablation}, removing pruning substantially increases over-budget games.
    ...
    \item Catastrophic-foul control prevents instant-loss fouls (illegal 8-ball potting; cue+8 scratch), improving robustness to action noise.
\end{itemize}
