...
\textbf{Environmental Noise:} Following real-world billiards, the simulator introduces Gaussian noise ($\sigma = 0.1$) to all action parameters, making outcome prediction inherently stochastic. Noise is applied independently in each action dimension in its native unit (degrees for angles, m/s for speed, and unitless for spin). Agents must be robust to this uncertainty.
...
\begin{itemize}
    ...
    \item Interestingly, adding a soft catastrophic-foul penalty can reduce win rate against BasicAgent in our setting; the default system relies on hard filtering and multi-stage verification to avoid instant-loss fouls, while the penalty term is treated as an optional conservative bias.
\end{itemize}
