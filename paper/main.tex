...
\textbf{Environmental Noise:} Following real-world billiards, the simulator introduces Gaussian noise ($\sigma = 0.1$) to all action parameters, making outcome prediction inherently stochastic. Agents must be robust to this uncertainty.
...
\begin{enumerate}
\item \textbf{Geometric-Guided Candidate Generation:} We develop an intelligent Ghost Ball method that generates high-quality shot candidates by exploiting billiards geometry, reducing the candidate pool from 450 to 15 while preserving solution quality—a 30$\times$ speedup in candidate generation.
...
\item \textbf{Engineering Evolution Narrative:} We document the evolution from an early heuristic baseline, through a naive evolution-strategy prototype that frequently violated time constraints, to the final balanced architecture, highlighting practical engineering trade-offs.
...
\begin{itemize}
    ...
    \item $R_{\text{strategic}} = w_{\text{pos}} \cdot f_{\text{pos}}(\mathbf{s}') + w_{\text{def}} \cdot f_{\text{def}}(\mathbf{s}')$
\end{itemize}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.48\textwidth]{figure1_pipeline.pdf}
\caption{Overall decision pipeline: (1) Ghost Ball candidate generation, (2) fast filtering (1--2 samples), (3) refined evaluation (2--4 samples) with catastrophic foul filtering, (4) selective CMA-ES refinement (triggered when score $< 60$), (5) positional/safety shot planning, and (6) final verification (4--6 samples) with increased sampling in high-risk states.}
\label{fig:pipeline}
\end{figure}
...
\textbf{Intelligent Pruning:} Naive enumeration over all target balls $\times$ all pockets $\times$ velocity/spin variants yields $\sim$450 candidates per decision. We formalize pruning as a filtering pipeline to construct a reduced candidate set $C_{\text{pruned}} \subset C_{\text{initial}}$:
...
\begin{enumerate}
    ...
    \item \textit{Path feasibility}: Reject candidate $(t, p)$ if $\exists b \in \mathcal{B} \setminus \{t\}$ such that $\text{dist}(\mathbf{p}_b, \text{ray}(\mathbf{p}_{\text{cue}}, \mathbf{p}_{\text{ghost}})) < 2.5R$, where ray denotes the line segment from cue ball to ghost position.
\end{enumerate}

This reduces candidates to $|C_{\text{pruned}}| \approx 15$ (a 30$\times$ reduction) while retaining high-quality shots. Additional velocity variants $V \in \{0.9V_0, 1.1V_0\}$ and spin configurations $\mathbf{s} \in \{(0,0), (0,\pm0.15)\}$ expand the final set to $\approx$15--20 candidates per decision.
...
CMA-ES maintains a Gaussian search distribution $\mathcal{N}(\mathbf{m}, \sigma^2 \mathbf{C})$, iteratively updating:
\begin{align}
\mathbf{m}^{(g+1)} &= \mathbf{m}^{(g)} + \sum_{i=1}^{\mu} w_i \mathbf{x}_i^{(g)} \\
\mathbf{C}^{(g+1)} &= (1 - c_{\text{cov}}) \mathbf{C}^{(g)} + c_{\text{cov}} \sum_{i=1}^{\mu} w_i (\mathbf{x}_i^{(g)} - \mathbf{m}^{(g)})(\mathbf{x}_i^{(g)} - \mathbf{m}^{(g)})^\top
\end{align}
...
$\alpha_{\text{align}} \in [0,1]$ measures alignment between cue-to-ball and ball-to-pocket vectors (higher when vectors are collinear). Constants $w_{\text{def}}$, $\beta_1$, and $\beta_2$ are empirically tuned (see Section VI-A).
...
\State \textbf{Stage 1: Intelligent Ghost Ball Generation}
...
\State \textbf{Stage 2: Fast Filtering (1--2 samples)}
...
\State \textbf{Stage 3: Refined Evaluation (2--4 samples)}
...
\State \textbf{Stage 6: Final Verification (4--6 samples)}
...
\end{itemize}
	...
	\textbf{Analysis:} Pure geometry provides strong candidate quality, but remains brittle under simulator stochasticity and does not explicitly manage risk or positioning.
...
\end{itemize}
	...
	\textbf{Analysis:} This milestone establishes the main compute/quality Pareto point: pruning improves throughput, and selective CMA-ES allocates extra compute only to hard decisions.

\textbf{Key Lesson:} \textit{Spend simulator calls on a small, high-quality candidate set; refine locally only when needed.}
...
Beyond the three milestones, we performed targeted micro-optimizations (threshold tuning, sampling schedules) and validated key components using ablations. Table~\ref{table_ablation} reports measured effects for removing pruning, disabling CMA-ES, disabling strategy/safety, and removing catastrophic-foul penalties.

\subsection{Final Architecture Summary}

The final agent combines:
\begin{enumerate}
    \item Intelligent Ghost Ball generation (15 candidates)
...
This pipeline achieves a Pareto-optimal balance between win rate and decision time, meeting the dual objectives of competitive performance and real-time responsiveness.
...
\textbf{Hardware:} Intel Xeon Gold 6248R CPU (3.0 GHz), 64 GB RAM, Ubuntu 22.04, Python 3.13.
...
Refined samples & 2--4 & Stage 3 \\
Final verification & 4--6 & Stage 6 \\
Critical state samples & 6 & 8-ball risk \\
\hline
...
\input{results/generated_table_winrate.tex}
...
\textbf{Analysis:}
\begin{itemize}
    \item Geometric pruning is essential for wall-clock feasibility under the 180s reference budget (see the over-180s column in Table~\ref{table_ablation}).
    \item Selective CMA-ES refinement improves win rate at a moderate runtime cost; disabling it reduces compute but also reduces decision quality.
    \item Strategy/safety terms stabilize play in low-reward states by preferring low-variance shots, especially when the 8-ball is close to a pocket.
    \item Catastrophic-foul control directly targets instant-loss outcomes (illegal 8-ball potting and cue+8 scratch), improving reliability under action noise.
\end{itemize}

\textbf{Synergy Effect:} The full system outperforms the sum of isolated components, indicating beneficial interactions (e.g., pruning + CMA-ES is more effective than either alone; strategic bonuses + safety planner reinforce defensive play).
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.9\textwidth]{figure5_case_study.pdf}
...
\begin{itemize}
    \item \textit{Environment setup and evaluation infrastructure:} Prepared the local evaluation environment, validated baseline agents, and supported tournament-style testing and reproducibility checks.
    \item \textit{Hyperparameter tuning and robustness improvements:} Performed targeted tuning (e.g., pruning thresholds, sampling schedules, and strategy weights) and proposed refinements that improved win rate and stability under noise.
    \item \textit{Experiment support and review:} Assisted with experimental design/verification and provided feedback on analysis and presentation for the final report.
\end{itemize}
...
The author thanks the AI3603 course staff at Shanghai Jiao Tong University for providing the Pooltool simulation environment and baseline agents. Special thanks to the developers of the Pooltool library for maintaining an open-source, high-fidelity billiards physics engine. This work was completed independently as part of the course project requirements.
